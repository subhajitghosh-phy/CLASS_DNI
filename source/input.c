...
/*------------------------------------------DNI------------------------------------------*/
class_test(ppt->gauge = synchronous , errmsg,"DNI: Only Newtonian Gauge is implemented")
/*---------------------------------------------------------------------------------------*/
    }
  }
...
/*.....................................................................DNI................................................................*/
  ...
  class_read_double("u_dni",ppt->u_dni);
  class_read_double("f_dni", pba->f_dni);
  class_test((pba->f_dni>1.0)||(pba->f_dni<0),
             errmsg,
             "Unphysical value of f_dni");
  class_test(ppt->u_dni<0,
             errmsg,
             "Unphysical value of f_dni");
...
/*.....................................................................DNI................................................................*/
  ...
  class_test(((flag1 == _TRUE_) || (flag2 == _TRUE_)),
             errmsg,
             "Omega_dm or omega_dm is expected instead of CDM");

  /** - Omega_0_dm (DM) */
  class_call(parser_read_double(pfc,"Omega_dm",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_dm",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_cdm or omega_cdm, choose one");
/*DNI* -- Omega0_cdm is set to the Omega0_cdm = Omega0_dm*(1-f) */
  if (flag1 == _TRUE_){
    pba->Omega0_cdm = param1*(1.0 - pba->f_dni); /*DNI*/
    pba->Omega0_dcdmdr = param1*pba->f_dni  /*DNI*/;
   }
  if (flag2 == _TRUE_) {
    pba->Omega0_cdm = param2/pba->h/pba->h*(1.0 - pba->f_dni);/*DNI*/
    pba->Omega0_dcdmdr = param2/pba->h/pba->h*pba->f_dni  /*DNI*/;
   }
...
/*DNI* Omega_0_dcdmdr is treated at neutrino interacting dark matter (NIDM). Omega0_dcdmdr = f/(1-f) * Omega0_cdm. */
/*  if (flag1 == _TRUE_){*/ /*DNI*/
/*  if (pba->f_dni != 0.0 )*/
/*    pba->Omega0_dcdmdr = pba->Omega0_cdm*(pba->f_dni/(1.0 - pba->f_dni));*/
/*			}*/
/*  if (flag2 == _TRUE_)*/
/*    pba->Omega0_dcdmdr = pba->Omega0_cdm*(pba->f_dni/(1.0 - pba->f_dni));*/
/*-----------------------------------------DNI--------------------------------------------*/
...
/*------------------------------------------DNI------------------------------------------*/
class_test(pba->K !=0 , errmsg," DNI: Non-zero curvature is not implimented yet.")
/*---------------------------------------------------------------------------------------*/
  /** - Set curvature sign */
...
int input_default_params(
  ...
  ppt->gauge=newtonian/*synchronous*/; /*DNI*/
  ...
  ppt->u_dni=0.0 ; /*DNI strength of interaction -- u */
  pba->f_dni = 0.0; /*DNI fraction of NIDM*/
  /*................................DNI...............................................*/
