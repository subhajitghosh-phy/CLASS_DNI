...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double velocity_tot;
  double s2_squared;
  /*DNI - Variables for setting initial conditions for DNI*/
  double loc_psi, frac_psi, frac_phi;
...
/*.............DNI : Setting initial conditions: For curvature K=0 ...............................*/
/* */
if (fabs(pba->f_dni*ppt->u_dni - 0.0)<=1.e-14) {   /*DNI - Lower value of u for which we switch back to LCDM initial condition */

/* DNI LCDM initial conditions for curvature K = 0 */

      loc_psi = (10.*ppr->curvature_ini)/(15.0 + 4.0*fracnu);
	
      ppw->pv->y[ppw->pv->index_pt_phi] = (1.+2./5.*fracnu)*loc_psi;

      ppw->pv->y[ppw->pv->index_pt_delta_g] = (-2./3.*(k*k)/(a_prime_over_a*a_prime_over_a)*ppw->pv->y[ppw->pv->index_pt_phi]  - 2.*loc_psi)*(1. + rho_m_over_rho_r )/ (1. + 0.75* rho_m_over_rho_r);

      ppw->pv->y[ppw->pv->index_pt_theta_g] = 0.5*k*k*loc_psi/a_prime_over_a*(1. + rho_m_over_rho_r )/ (1. + 0.75* rho_m_over_rho_r); 

      ppw->pv->y[ppw->pv->index_pt_delta_b]  = (3.0/4.0)*ppw->pv->y[ppw->pv->index_pt_delta_g];
      ppw->pv->y[ppw->pv->index_pt_theta_b]  = ppw->pv->y[ppw->pv->index_pt_theta_g];

      if (pba->has_cdm == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_cdm] = (3.0/4.0)*ppw->pv->y[ppw->pv->index_pt_delta_g];
        ppw->pv->y[ppw->pv->index_pt_theta_cdm] = ppw->pv->y[ppw->pv->index_pt_theta_g];

      }

      if (pba->has_dcdm == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_dcdm] = (3.0/4.0)*ppw->pv->y[ppw->pv->index_pt_delta_g];
        ppw->pv->y[ppw->pv->index_pt_theta_dcdm] =  ppw->pv->y[ppw->pv->index_pt_theta_g];
      }
      ...
      if ((pba->has_ur == _TRUE_) || (pba->has_ncdm == _TRUE_) || (pba->has_dr == _TRUE_)) {
        ...
        delta_ur = ppw->pv->y[ppw->pv->index_pt_delta_g];
        theta_ur = ppw->pv->y[ppw->pv->index_pt_theta_g];
        shear_ur = k/fracnu*k/6.*(ppw->pv->y[ppw->pv->index_pt_phi] - loc_psi)/(a_prime_over_a*a_prime_over_a);
    ...
    }/*DNI closing the LCDM initial condition loop*/


      else {
 /* Starting the DNI initial conditions: In this case the only difference wrt to LCDM case is that initial neutrino shear is set to zero as they were coupled to NIDM.*/
	
      ppw->pv->y[ppw->pv->index_pt_phi] = (10.*ppr->curvature_ini)/(15.0 );

/* DNI -- In this case the two newtonian potentials are equal. /phi = /psi */

      ppw->pv->y[ppw->pv->index_pt_delta_g] = (-2./3.*(k*k)/(a_prime_over_a*a_prime_over_a) - 2.)*ppw->pv->y[ppw->pv->index_pt_phi]*(1. + rho_m_over_rho_r )/ (1. + 0.75* rho_m_over_rho_r);
      ppw->pv->y[ppw->pv->index_pt_theta_g] = 0.5*k*k*ppw->pv->y[ppw->pv->index_pt_phi]/a_prime_over_a*(1. + rho_m_over_rho_r )/ (1. + 0.75* rho_m_over_rho_r);
    ...
    }/*closing the loop of DNI initial conditions*/
...
int perturb_einstein(
  ...
  if (_tensors_) {
    ...
    /* single einstein equation for tensor perturbations *//*tensor.eq*/
...
int perturb_derivs(double tau,
 ...
 /*.......................................DNI................................................*/
 if (pba->has_dcdm == _TRUE_) {
  mu_dot = a*ppt->u_dni*pvecback[pba->index_bg_rho_dcdm]*1.9358;
  s_inv = (4*pvecback[pba->index_bg_rho_ur])/(3*pvecback[pba->index_bg_rho_dcdm]);
  }
 else{
  mu_dot = 0.0; /*will be useful for tensor evolution-- no need to code LCDM seperately*/ 
	}
/*..........................................................................................*/
  ...
  if (_scalars_) {
    ...
    if (pba->has_dcdm == _TRUE_) {
      ...
      dy[pv->index_pt_theta_dcdm] = - a_prime_over_a*y[pv->index_pt_theta_dcdm] + metric_euler /*DNI*/ - s_inv*mu_dot*(y[pv->index_pt_theta_dcdm] - y[pv->index_pt_theta_ur]); /* dcdm velocity */
...
/*----------------------------------------------DNI---------------------------------------*/
  if (pba->has_dcdm == _TRUE_) {
        dy[pv->index_pt_theta_ur] =
          ...
          -(1.-ppt->three_ceff2_ur)*a_prime_over_a*y[pv->index_pt_theta_ur] /*DNI*/ 
	+ mu_dot*(y[pv->index_pt_theta_dcdm] - y[pv->index_pt_theta_ur]);
	}
  else{
	dy[pv->index_pt_theta_ur] =
          // standard term with extra coefficient (3 ceff2_ur), normally equal to one
	...
	if (pba->has_dcdm == _TRUE_) {
          ...
          dy[pv->index_pt_shear_ur] =
            0.5*(
                 ...
                 -(1.-ppt->three_cvis2_ur)*(8./15.*(y[pv->index_pt_theta_ur]+metric_shear))) /*DNI*/ - mu_dot*y[pv->index_pt_shear_ur] ;

          /** - -----> exact ur l=3 */
          l = 3;
          dy[pv->index_pt_l3_ur] = k/(2.*l+1.)*
            (l*2.*s_l[l]*s_l[2]*y[pv->index_pt_shear_ur]-(l+1.)*s_l[l+1]*y[pv->index_pt_l3_ur+1]) /*DNI*/ - mu_dot*y[pv->index_pt_l3_ur];

          /** - -----> exact ur l>3 */
          for (l = 4; l < pv->l_max_ur; l++) {
            dy[pv->index_pt_delta_ur+l] = k/(2.*l+1)*
              (l*s_l[l]*y[pv->index_pt_delta_ur+l-1]-(l+1.)*s_l[l+1]*y[pv->index_pt_delta_ur+l+1]) /*DNI*/ - mu_dot*y[pv->index_pt_delta_ur+l];
          }

          /** - -----> exact ur lmax_ur */
          l = pv->l_max_ur;
          dy[pv->index_pt_delta_ur+l] =
            k*(s_l[l]*y[pv->index_pt_delta_ur+l-1]-(1.+l)*cotKgen*y[pv->index_pt_delta_ur+l]) /*DNI*/ - mu_dot*y[pv->index_pt_delta_ur+l];
	}
	else {
	/** - -----> exact ur shear */
          dy[pv->index_pt_shear_ur] =
...
/*................................DNI.tensor......................................................*/
                    ...
                    PU = 0 /*-1.0/_SQRT6_*(y[pv->index_pt_delta_ur])*/;

      dy[pv->index_pt_delta_ur] = -4./3.*y[pv->index_pt_theta_ur]+_SQRT6_*y[pv->index_pt_gwdot] + /*DNI*/  - mu_dot*(y[pv->index_pt_delta_ur] + _SQRT6_*PU) ;

      dy[pv->index_pt_theta_ur] = k2*(y[pv->index_pt_delta_ur]/4.-s2_squared*y[pv->index_pt_shear_ur]) /*DNI*/ - mu_dot*y[pv->index_pt_theta_ur];

      dy[pv->index_pt_shear_ur] = (4./15.*y[pv->index_pt_theta_ur]
                                   -3./10.*k*s_l[3]/s_l[2]*y[pv->index_pt_shear_ur+1])/*DNI*/ - mu_dot*y[pv->index_pt_shear_ur];
